        ...
        <main className="flex h-screen w-full overflow-hidden font-sans">
            ...
            <div className="w-[35%] h-full shrink-0 z-10 relative shadow-xl bg-slate-900 flex flex-col">
                ...
                <div className="flex border-b border-slate-800">
                    <button
                        onClick={() => setMode('ai')}
                        className={`flex-1 flex items-center justify-center gap-2 py-4 text-sm font-medium transition-all ${mode === 'ai'
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-4 text-sm font-medium transition-all ${mode === 'form'
                            ? 'bg-slate-800 text-blue-400 border-b-2 border-blue-500'
                            : 'text-slate-500 hover:text-slate-300'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-4 text-sm font-medium transition-all ${mode === 'salesforce'
                            ? 'bg-slate-800 text-blue-400 border-b-2 border-blue-500'
                            : 'text-slate-500 hover:text-slate-300'
                            }`}
                    >
                    ...
                    <div className="flex-1 overflow-hidden">
                        {mode === 'ai' ? (
                            ...
                            <div className="h-full flex flex-col justify-center items-center p-8 text-center bg-slate-900">
                                ...
                                <h2 className="text-xl font-bold text-white mb-2">Salesforce Integration</h2>
                                <p className="text-slate-400 text-sm mb-6">Simulation mode for automated opportunity triggers.</p>
                                <div className="w-full max-w-sm">
                                    <SalesforceSimulator onLoadState={handleWizardUpdate} />
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="w-[65%] h-full">
                <Preview input={input} result={result} onUpdateField={handleInputChange} />
            </div>
        </main>
    );
}
